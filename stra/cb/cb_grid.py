...
class cbGridStrategy(bt.Strategy):
    """
    继承并构建新的网格策略
    """
    # print('参数设定')
    ...
    def __init__(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            # self.date[d._name] = -1
        self.n = int(len(self.datas)/10)
        self.order = None
        self.c = self.p.period
        self.check = None
    ...
    def get_check(self):
        ...
        for i, d in enumerate(self.datas):
            # 记录可转债的转股溢价率
            tmp_d[d._name] = d.transprice[0]
        # 按照value进行排序 从小到大
        sort_list = sorted(tmp_d.items(), key=lambda x: x[1])
        down_list = sort_list[:int(len(sort_list)*0.1)]
        check_list = [c[0] for c in down_list]
        self.c = 0
        return check_list

    def next(self):
        if self.c == self.p.period:
    ...
    def market_in(self, security, current_price, p_value, cash):
        ...
        if cost_order > cash:
            cost_order = cash
        # 买多少手
        unit_order1 = int(cost_order / current_price / 10) * 10
    ...
    def stop(self):
        ...
        # order_p.to_csv('D:/大学/A研一/量化金融/system/history/cborder_history.csv',index=False)
        # trader_p.to_csv('D:/大学/A研一/量化金融/system/history/cbtrader_history.csv', index=False, encoding='utf_8_sig')